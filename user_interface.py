...
#Personal Info section
    def personal_info (self):
        ...
        def name (self):
            label_name = tk.Label(self.second, text ="Name: ") #For the user's name
            label_name.grid(row = 2, column = 0, padx = 5, pady = 5)
            self.input_name = tk.Entry(self.second)		#For the user's name
            self.input_name.grid(row = 2, column = 1, padx = 5, pady = 5)
        #Defining method for Age's data
        def age (self):
            label_age = tk.Label(self.second, text ="Age: ") #For the user's age
            label_age.grid(row = 3, column = 0, padx = 5, pady = 5)
            self.input_age = tk.Entry(self.second)		#For the user's age
            self.input_age.grid(row = 3, column = 1, padx = 5, pady = 5)	
        #Defining method for Address' data
        def address (self):
            label_address = tk.Label(self.second, text ="Address: ") #For the user's address
            label_address.grid(row = 4, column = 0, padx = 5, pady = 5)
            self.input_address = tk.Entry(self.second)		#For the user's address
            self.input_address.grid(row = 4, column = 1, padx = 5, pady = 5)	
        #Defining method for Number's data
        def number (self):
            label_number = tk.Label(self.second, text ="Phone number: ") #For the user's phone number
            label_number.grid(row = 5, column = 0, padx = 5, pady = 5)
            self.input_number = tk.Entry(self.second)		#For the user's number
    ...
    def health_info (self):
        self.section2_title = tk.Label(self.third, text="Health Declaration", font=("Courier 10 pitch", 12), bg=self.third['bg']) #Section Title
        self.section2_title.grid(row=7, column=0, columnspan=3, sticky='ew', padx=10)
    ...
    def vaccination_status (self):
        label_vaccine = tk.Label(self.third, text ="Have you been vaccinated for COVID-19?") #Question label
        ...
        for i in range (len(vac_status)):
            self.input_vaccine = tk.Radiobutton(self.third, text=vac_status[i], variable = self.var, value = vac_status[i]) 
            self.input_vaccine.grid(row = 9+i, column = 0, padx = 0, pady = 0, sticky = "w")	     
    #Second Question (Symptoms)
    def get_symptoms(self):
        label_vaccine = tk.Label(self.third, text="Are you experiencing any symptoms in the past 7 days such as:")
        ...
        for i, option in enumerate(options):
            var = tk.StringVar()
            input_symptom = tk.Checkbutton(self.third, text=option, variable=var, onvalue = option, offvalue="", anchor = "w")
    ...
    def exposure(self):
        label_exposure = tk.Label(self.third, text ="Have you had exposure to a probable or confirmed case in the last 14 days?")#Question label
        ...
        for i in range (len(ex_status)):
            self.input_exposure = tk.Radiobutton(self.third, text=ex_status[i], variable = self.ex_var, value = ex_status[i]) 
            self.input_exposure.grid(row = 22+i, column = 0, padx = 0, pady = 0, sticky = "w")
    #Fourth Question (Contact with someone with symptoms)
    def contact (self):
        label_contact = tk.Label(self.third, text="Have you had in contact with somebody with body pains\n"
        ...
        for i in range (len(contact_status)):
            self.input_contact = tk.Radiobutton(self.third, text=contact_status[i], variable = self.contact_var, value = contact_status[i])
            self.input_contact.grid(row=25+i, column = 0, padx = 0, pady = 0, sticky ="w")
    #Fifth Question (If tested for COVID)
    def test (self):
        label_test = tk.Label(self.third, text="Have you been tested for COVID-19 in the last 14 days?") #Question label
        ...
        for i in range (len(test_status)):
            self.input_test = tk.Radiobutton(self.third, text = test_status[i], variable = self.test_var, value = test_status[i]) 
